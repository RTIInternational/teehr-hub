...
import { useRetrospectiveFilters } from '../../../hooks/useRetrospectiveDataFetching';
import { useRetrospectiveData } from './useRetrospectiveData';

const MapFilterButton = () => {
  const [showDropdown, setShowDropdown] = useState(false);
  const { state } = useRetrospectiveDashboard();
  const { mapFilters, updateMapFilters } = useRetrospectiveFilters();
  const { loadLocations } = useRetrospectiveData();
  
  // Debug: Log what state.configurations actually is
  console.log('MapFilterButton state:', {
    configurations: state.configurations,
    configurationsType: typeof state.configurations,
    isArray: Array.isArray(state.configurations)
  });
